...
{
    public static void LoginToDocker()
    {
    ...
    /// </summary>
    public static async Task Deploy(this AspireSolution aspire)
    {
        LoginToDocker();

        var deployment = new KubernetesDeployment(aspire);
        var result = await deployment.CheckNamespace();
        AnsiConsole.MarkupLine($"[green]{result} namespace {deployment.Namespace} in k8s.[/]");
